...
public class User {
	...
	public User(String userId, String userPw, String userName, int userAge,
		...
		// 왜 사용하느가
		// 중복 코드 제거, 코드 길이감소, 자사용성 증가..
		// 가독성이 좀 떨어지는 경우가 있다(단점)
		
		this(userId, userPw); // 기본생성자
		// this.userId = userId;
		// this.userPw = userPw;
	...
	//public User() {} // 기본생성자가 이미 적상 됨
	
	public User(String userId) {} // 매개변수의 개수가 같은 생성자 없음
	// -> 오버로딩 성립
	
	public User(int userAge) {} // 매개변수의 갯수가 같지만 타입이 다름
	// -> 오버로딩 성립
	
	public User(String userId, int userAge) {}
	// 매개변수의 갯수는 같지만 String, int 순서의 생성자는 존재치 않아
	// -> 오버로딩 성립
