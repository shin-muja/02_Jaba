...
public class StudentService {
	...
	// * 인터페이스 객체생성 x,  부보 참조변수 0
	
	// 구현체 : java.util.ArrayList : 배열 형태 List(가장 대표적인 List 자식 클래스)
	...
	public StudentService() {
		...
		studentList.add(new Student("고영희", 22, "경기도 안산시", 'F', 100)); 
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				case 3 : System.out.println(updateStudeng() ); break;
				...
				System.out.println("\nerror : 입력형식이 휴효하지 않습니다. 다시 시도해주세요");
				menuNum = -1; // 첫 반복 시 잘못 입력하는 경우에
							// menuNum이 0을 가지고 있어 종료되는데,
							// 이를 방지하기 위해 임의값 -1 대입
				sc.nextLine(); // 입력버퍼에 남아있는 잘못된 코드제거
			}
	...
	/** 1. 학생 정보 추가 메서드
	 * @return 상비 성공 시 "성공" 실패 시 "실패" 문자열 반환
	 */
	public String addStudent() throws InputMismatchException {
		...
		char gender = sc.next().toUpperCase().charAt(0); // 'M' or 'F'
	...
	public void selectAll() {
		...
		if( studentList.isEmpty()) { // boolean isEmpty() : 컬렉션이 비어있다면 true반환
		...
		*/
		...
		// 작성법
		// for( 컬렉션 또는 배열에서 꺼낸 한개의 요소를 저장할 변수 : 반복접근할 대상(컬렉션명 또는 배열명))
		// {수행할 내용}
		int index = 0;
	 ...
	 * @return
	 */
	public String updateStudeng() throws InputMismatchException {
		...
		// 4) 만약 문자열을 입력한 경우 -> 예외처리 throws
			...
			System.out.println(index + "번쨰 저장된 학생 정보");
			...
			// index 번째에 있던 기존 객체 정보가 반환되고, 그 객체를 temp에 저장
	...
	public String removeStudent() throws InputMismatchException{
		...
		// 4) 만약 문자열을 입력한 경우 -> 예외처리 throws
			...
			if(ch == 'Y') {				
				return studentList.remove(index).getName() + "의 정보가 삭제되었습니다";
	 ...
	 * 문자열 입력받아 studentList에서 꺼내온 Studnt의 객체의
	...
	public void searchName2() {
		...
		System.out.print("이름에 포함되는 문쟈열 이름 입력 : ");
		String input = sc.next();
		...
		for( Student std : studentList ) {
			...
			// 순서 유의 input.contains(std.getName)); 포함 안되는 경우가 많음
	...
	public void sortByName() {
		...
		// -> 이 메서드를 배교으 ㅣ키로 사용하여 각 Student 객체를 비교하고 정렬함.
	...
	// private List<Object> testList = new ArrayList<Object>(); // 업캐스팅 적용
	...
	// 제네릭에 Object 기입 시 모든 객체를 받겠다는 뜻
	/*
	public void ex() {
		// List 테스트용 예제
		...
		testList.add(new Student("홀깅동", 25, "서울시 중구", 'M', 90));
		 testList.add("문자열");
		 testList.add(sc);
		...
		// testList 꺼내온 인덱스 요소고 Strudent 혹은 상속관계인지 검사
