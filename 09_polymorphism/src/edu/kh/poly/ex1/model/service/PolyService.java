...
public class PolyService {
	...
	// 자식 참조변수 = 자식객체
		...
		// 객체 배열 : 같은 객체 참조 자료형의 변수를 하나의 묶음
		// 다형성을 이요한 객체배열
	...
	// 전달받은 Car또는 자식객체(Teslam Spark)의 엔진, 연료, 바퀴갯수를 출력 메서드
	...
	public void ex3() {
		...
		// 주의 ! : "."연산자가 형변환 연산자보다 연산 우선 순위가 높다
		// 그래서 형변환하는 부분을 () 로 한번 묶어주고나서 . 연산을 진행해야
		// Tesla 객체의 getBatteryCapacity()를 사용할수 있음.
	...
	public void ex4() {
		...
		Car c1 = new Tesla(); // Tesla 객체를 만들어 Car타입(부모타입) C1에 담음
		
		//Spark s1 = (Spark)c1; // c1을 관계없는 Spark로 다운캐스팅하려고 함
		// Type mismatch: cannot convert from Car to Spark
		// 다운캐스팅 잘못해도 컴파일 에러 발생 안함
		// -> 실행 시 발생(런타임 에러)
		// ClassCastExcoption : 형변환 예외
		
		// Runtime error : 프로그램 실행 시 발생하는 예외(에러)
		// compile error : 싨행 전 코드에 빨간줄 에러
	...
	public void ex5() {
		...
		// Car 객체에 있는 toString() 메소드를 호출 == 정적바인딩
		// 프로그램 "실행 전"
