...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		int[] arr; // == int arr[];
		// Stack 영역에 int[] 자료형 공간을 4byte할당하고
		// 그 공간에 arr이라는 이름을 부여
		...
		// new int[3] : heap 영역에 int3칸짜리 int[]을 생성(할당)
		...
		System.out.println(arr[0] = 20); // 10 -> 20
	...
	public void ex2() {
		...
		arr[2] = 500;
		arr[3] = 1000;
		// arr[4] = 5000; error : Index 4 out of bounds for length 4
		// 배열의 인덱스 범위 벗어남 : 길이가 4칸짜리 배열인데,
		// 없는 인덱스번호인 4에 접근하려해서 발생하는 error
	...
	public void ex3() {
		...
		// double[] 자료형 자료형 참조변수 height를 stack영역에 생성하고
	...
	public void ex4() {
		// 입력받은 인원 수 만큼의 정수를 입력 받아 배열에 저장
		...
		int poeple = sc.nextInt();
		
		// 배열 선언 및 할당
		// 할당할 배열의 크기는 입력받은 인원수 크기 만큼(input)
		int score[] = new int[poeple];
	...
	public void ex5() {
		...
		// Arrays.toStirng(배열명) : 모든 요소 값을 출력
		System.out.println(Arrays.toString(arr));
	...
	public void ex6() {
		...
		System.out.println("오늘 점시 메뉴 : " + arr[(int)(Math.random() * 6)]);
	...
	public void ex7() {
		...
		// flag == true : 일치하는 존재 값이 존재
		
		boolean flag = false; // 검사전에는 업다고 가정
		...
		// -> ! (Not 연산자 : 부정 논리 연산자 ) 붙여줌으로 써 true로 변경하여
		// {} 안에 있는 코드를 수행하게 함.
	...
	public void ex9() {
		...
		// sc.next(0 -> String 형
		// Stirng.charAt(0) -> String의 0번쨰 인덱스 문자 하나를 추출해서 char 형태로 반환
		// "h" -> 'h'
		...
		if( count > 0) {
			...
			System.out.println("동일한 문자가 존재하지 않습니다");
		}
